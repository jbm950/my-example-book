...
use stm32f4xx_hal::{
    ...
    rcc::{self, RccExt},
    timer::{CaptureChannel, CaptureExt, CaptureHzManager, Event, Flag, PwmExt, TimerExt},
};
...
struct SharedResources {
    ...
    capture_manager: CaptureHzManager<pac::TIM3>,
...
fn TIM3() {
    critical_section::with(|cs| {
        ...
        if shared.capture_manager.flags().contains(Flag::C1Overcapture) {
            shared.prev_count = None;
            shared.capture_manager.clear_flags(Flag::C1Overcapture);
            return;
        }
        ...
        if shared.sample_index != TOTAL_SAMPLES {
            ...
            shared.avg_rpm =
                shared.samples.iter().sum::<u32>() as f64 / shared.samples.len() as f64;

            shared.sample_index = 0;
            shared.capture_complete = true;
            shared.prev_count = None;
...
fn report_capture_once_complete(duty: u8) {
    loop {
        let avg_rpm = critical_section::with(|cs| {
            let mut shared = SHARED_RESOURCES.borrow(cs).borrow_mut();
            let Some(shared) = shared.as_mut() else {
                return Some(0.0);
            };

            if shared.capture_complete {
                shared.capture_complete = false;
                Some(shared.avg_rpm)
            } else {
                None
            }
...
fn main() -> ! {
    ...
    let capture = capture_ch1.with(capture_pin);
