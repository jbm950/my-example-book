...
use crate::{net::TcpRt, protocol::{DataWord, StatusMessage, StatusWord, Subaddress, TxRx}};
...
pub async fn rt(server: SocketAddr, rt_id: u8) -> io::Result<()> {
    ...
    loop {
        ...
        match cmd_msg.word.subaddr {
            Subaddress { address: 5, tr: TxRx::R } => {
                info!("Received 5R command with data: {:?}", cmd_msg.data);
                rt.write(StatusMessage { word: status_word, data: Vec::new() }).await?;
            }

            Subaddress { address: 7, tr: TxRx::T } => {
            ...
            Subaddress { address: 13, tr: TxRx::T } => {
            ...
            subaddr => {
                error!("Unimplemented subaddress {} TxRx {:?}", subaddr.address, subaddr.tr);
